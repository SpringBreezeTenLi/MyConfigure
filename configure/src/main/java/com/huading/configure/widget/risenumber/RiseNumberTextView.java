...
import com.huading.configure.utils.StringUtil;
    ...
    private void runFloat() {
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                if (flags) {
                    setText(new StringUtil().format("##0.00").format(Double.parseDouble(valueAnimator.getAnimatedValue().toString())));
                    if (valueAnimator.getAnimatedValue().toString().equalsIgnoreCase(number + "")) {
                        setText(new StringUtil().format("##0.00").format(Double.parseDouble(number + "")));
                    }
                } else {
                    setText(new StringUtil().format("##0.00").format(Double.parseDouble(valueAnimator.getAnimatedValue().toString())));
                    if (valueAnimator.getAnimatedValue().toString().equalsIgnoreCase(number + "")) {
